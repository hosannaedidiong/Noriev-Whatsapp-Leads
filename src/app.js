...
async function handleIncoming(incoming) {
  const existing = (await store.getSession(incoming.from)) || newSession();
  ...
  await store.saveSession(incoming.from, session);

  if (leadComplete) {
    await store.saveLead(incoming.from, session.lead);
...
// Demo/admin: view captured leads without a DB client.
